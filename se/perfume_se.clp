...
(defrule ask-for-preference
  (user-input ?notes)
  =>
  (printout t "¿Que notas altas prefieres en un perfume? (neroli/pera/bergamota/manzana/cardomo/piña/melon/limon)" crlf)
  (assert (hnotes-input (read)))
)
...
(defrule recommend-other-based-perfume
    (gender-input ?gender)
    (hnotes-input ?hnote)
    (perfume (gender ?gender)(high-notes ?hnote)
    (name ?name))
    =>
    (printout t "Basado en tu genero te recomendamos: " ?name "." crlf)
)
